window.fathom = (function () {
  ...
  function qs() {
    for (var pair, data = {}, pairs = window.location.search.substring(window.location.search.indexOf("?") + 1).split("&"), i = 0; i < pairs.length; i++)
      pairs[i] &&
        ((pair = pairs[i].split("=")), -1 < ["keyword", "q", "ref", "s", "utm_campaign", "utm_content", "utm_medium", "utm_source", "utm_term", "action", "name", "pagename", "tab", "via"].indexOf(decodeURIComponent(pair[0]))) &&
  ...
  function trackingEnabled() {
    ...
    if (!fathomIsBlocked && !prerender && !honorDNT && !isExcludedDomain && isAllowedDomain) return !0;
  }
  function getLocation(params) {
    ...
    return (
      void 0 === params.url
        ? canonical && document.querySelector('link[rel="canonical"][href]') && (((a = document.createElement("a")).href = document.querySelector('link[rel="canonical"][href]').href), (location = a))
        : ((location = document.createElement("a")).href = params.url),
      location
    );
  }
  return (
    fathomScript.src.indexOf("cdn.usefathom.com") < 0 && (((scriptUrl = document.createElement("a")).href = fathomScript.src), (trackerUrl = "https://" + scriptUrl.hostname + "/")),
    auto &&
      setTimeout(function () {
        window.fathom.trackPageview();
      }),
    {
      ...
      trackPageview: function (params) {
        ...
        "" !== location.host &&
          ...
          this.send({ h: hostname, p: pathnameToSend, r: params.referrer || (document.referrer.indexOf(hostname) < 0 ? document.referrer : ""), sid: this.siteId, qs: JSON.stringify(qs()) }));
      },
      trackGoal: function (code, cents) {
        var location = getLocation({}),
          hostname = location.protocol + "//" + location.hostname;
        this.beacon({ gcode: code, gval: cents, qs: JSON.stringify(qs()), p: location.pathname || "/", h: hostname, r: document.referrer.indexOf(hostname) < 0 ? document.referrer : "", sid: this.siteId });
      },
      trackEvent: function (name, payload = {}) {
        var location = getLocation({}),
          hostname = location.protocol + "//" + location.hostname;
        this.beacon({ name: name, payload: JSON.stringify(payload), p: location.pathname || "/", h: hostname, r: document.referrer.indexOf(hostname) < 0 ? document.referrer : "", sid: this.siteId, qs: JSON.stringify(qs()) });
      },
      setSite(siteId) {
        this.siteId = siteId;
      },
